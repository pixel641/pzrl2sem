...
private:
    ...
    //! Хеш-функция
    size_t hash_function(const KeyType &key) const; // std::hash<std::string>

    size_t get_index(const size_t &hash) const;

    void insert_to_table(const KeyType &key, const ValueType &value);
};